...
/** Mirrors the public feed: filter pills, featured card, compact list, optional sidebars. */
export function FeedSkeleton({ layout = "sidebar-right" }: { layout?: string }) {
  const showLeft  = layout === "sidebar-left"  || layout === "sidebar-both"
  const showRight = layout === "sidebar-right" || layout === "sidebar-both"
  const hasSidebar = showLeft || showRight

  const mainContent = (
    <div className={hasSidebar ? "min-w-0 flex-1" : undefined}>
    ...
    <div>
      ...
      {hasSidebar ? (
        <div className="flex items-start gap-8">
          {showLeft && <SidebarSkeleton />}
          {mainContent}
          {showRight && <SidebarSkeleton />}
        </div>
      ) : mainContent}
    </div>
  )
