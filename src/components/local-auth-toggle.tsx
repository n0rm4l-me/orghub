...
import { toggleLocalAuth } from "@/lib/actions/settings"
...
export function LocalAuthToggle({ initialEnabled }: Props) {
  const [enabled, setEnabled] = useState(initialEnabled)
  const { run, pending } = useAction(toggleLocalAuth, {
  ...
  return (
    <button
      type="button"
      onClick={() => {
          if (enabled && !confirm("Disabling password login will lock out all local accounts, including yours. Continue?")) return
          run(!enabled)
        }}
      disabled={pending}
      aria-pressed={enabled}
      aria-label={enabled ? "Disable password login" : "Enable password login"}
